...
/// </summary>
internal class CustomSQSPublisher : ISQSPublisher
{
    ...
    /// <exception cref="InvalidMessageException">If the message is null or invalid.</exception>
    /// <exception cref="MissingMessageTypeConfigurationException">If cannot find the publisher configuration for the message type.</exception>
    public async Task<SQSSendResponse> SendAsync<T>(T message, SQSOptions? sqsOptions,
        CancellationToken token = default)
    {
        using (var trace = _telemetryFactory.Trace("Publish to AWS SQS"))
        {
            try
            {
                trace.AddMetadata(MyTelemetryKeys.ObjectType, typeof(T).FullName!);

                _logger.LogDebug("Publishing the message of type '{MessageType}' using the {PublisherType}.", typeof(T),
                    nameof(CustomSQSPublisher));
                ...
                trace.AddMetadata(MyTelemetryKeys.MessageId, messageEnvelope.Id);
                ...
                _logger.LogDebug("Sending the message of type '{MessageType}' to SQS. Publisher Endpoint: {Endpoint}",
                    typeof(T), queueUrl);
    ...
    private string GetPublisherEndpoint(ITelemetryTrace trace, Type messageType, SQSOptions? sqsOptions)
    {
        ...
        if (mapping is null)
        {
            _logger.LogError("Cannot find a configuration for the message of type '{MessageType}'.",
                messageType.FullName);
            throw new MissingMessageTypeConfigurationException(
                $"The framework is not configured to accept messages of type '{messageType.FullName}'.");
        }

        if (mapping.PublishTargetType != PublisherTargetType.SQS_PUBLISHER)
        {
            _logger.LogError("Messages of type '{MessageType}' are not configured for publishing to SQS.",
                messageType.FullName);
            throw new MissingMessageTypeConfigurationException(
                $"Messages of type '{messageType.FullName}' are not configured for publishing to SQS.");
        }
        ...
        {
            _logger.LogError("Unable to determine a destination queue for message of type '{MessageType}'.",
                messageType.FullName);
            throw new InvalidPublisherEndpointException(
                $"Unable to determine a destination queue for message of type '{messageType.FullName}'.");
        }

        trace.AddMetadata(MyTelemetryKeys.MessageType, mapping.MessageTypeIdentifier);
        trace.AddMetadata(MyTelemetryKeys.QueueUrl, queueUrl);
